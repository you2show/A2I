...
const DICT: Record<string, { km: string; en: string }> = {
  ...
  presetVllm: { km: 'vLLM (GPU, ក្នុងម៉ាស៊ីន)', en: 'vLLM (GPU, local)' },
  presetOllama: { km: 'Ollama (ក្នុងម៉ាស៊ីន)', en: 'Ollama (local)' },
  ...
  noteSmallModel: { km: '⚠️ ម៉ូដែលតូច ({model}) សរសេរខ្មែរមិនល្អទេ — ចម្លើយអាចមើលទៅត្រឹមត្រូវ តែពិតជាខុស។ សូមប្ដូរទៅម៉ូដែលធំជាង (Qwen2.5 7B ក្នុងបញ្ជីខាងលើ), A2I Core, ឬ ✨ Gemini', en: '⚠️ Small model ({model}) cannot write Khmer reliably — answers can look fluent but be invented. Switch to a larger model (Qwen2.5 7B in the list above), A2I Core, or Gemini.' },
  noteBrainOffline: { km: '🖥️ {name} មិន online ទេ — ប្ដូរទៅ 🧠 In-browser AI ដោយស្វ័យប្រវត្តិ', en: '🖥️ {name} is offline — switching to the in-browser AI' },
  ...
  projStartCore: { km: 'ចាប់ផ្ដើម A2I Core (cd a2i-core && ./run.sh) រួចបន្ថែមក្នុង ⚙️ Settings', en: 'Start A2I Core (cd a2i-core && ./run.sh), then add it in ⚙️ Settings.' },
